    ...
    jda.updateCommands()
        .addCommands(
            ...
            Commands.slash("queue", "get the current music queue")
                .addOption(OptionType.INTEGER, "page", "queue page number"),
            Commands.slash("skip", "skip current track"),
            Commands.slash("repeat", "enable repeat mode"),
            Commands.slash("roll", "ROLL THE BONES AND DECIDE THY FATE")
                .addOption(
                    OptionType.STRING,
                    "dice",
                    "Dice to roll, supports #d# + #. modifier expressions (8d6!, 8d6kh2, 8d6d2 etc) not supported"),
            ...
            Commands.slash("jumpqueue", "plays the given track immediately")
                .addOption(OptionType.STRING, "url", "youtube url"))
        .queue();

    Collection<Integer> numbers = Arrays.asList(1, 2, 1, 3);
    for (int number : findUniqueNumbers(numbers)) System.out.println(number);
  }
